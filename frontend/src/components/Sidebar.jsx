...
export default function Sidebar({ isCollapsed, onToggleCollapse }) {
    ...
    fetch(`${API_BASE}/api/applications`)
      ...
      .then((apps) => {
        setStats({
          ...
          avgScore: apps.length > 0 ? Math.round(apps.reduce((s, a) => s + (a.match_score || 0), 0) / apps.length * 100) : 0,
        });
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <aside
      className={`bg-white border-r border-gray-100 flex flex-col transition-all duration-300 ease-in-out flex-shrink-0 overflow-hidden ${
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center rounded-xl text-[13px] font-medium transition-all duration-150 group ${
        ...
        <div className="px-4 py-3 border-t border-gray-100">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              <div key={s.label} className="flex flex-col items-center py-1.5 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors cursor-default">
      ...
      <div className={`border-t border-gray-100 ${isCollapsed ? "px-2 py-3" : "px-3 py-3"}`}>
        <button
          ...
          className={`w-full flex items-center rounded-xl text-[13px] text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-all duration-150 ${
            isCollapsed ? "justify-center px-0 py-2.5" : "gap-3 px-3 py-2.5"
          }`}
        >
          {isCollapsed ? (
      ...
      {!isCollapsed && (
        <div className="px-3 pb-3">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors cursor-default">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0 shadow-sm">
              {initials}
            </div>
            ...
            <button
              ...
              title="Sign out"
              className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-300 hover:bg-red-50 hover:text-red-500 transition-colors flex-shrink-0"
            >
      ...
      {isCollapsed && (
        <div className="px-2 pb-3 flex flex-col items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white text-[11px] font-bold shadow-sm">
            {initials}
          </div>
          <button
            onClick={handleLogout}
