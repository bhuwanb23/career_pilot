...
const navItems = [
  ...
  { label: "Pipeline", path: "/pipeline", icon: <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5m.75-9l3-3 2.148 2.148A12.061 12.061 0 0116.5 7.605" /></svg> },
  { label: "Approved", path: "/approved", icon: <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
];
...
export default function Sidebar({ isCollapsed, onToggleCollapse }) {
  ...
  return (
    <aside
      className={`bg-white border-r border-figma-hairline flex flex-col transition-all duration-300 ease-in-out flex-shrink-0 overflow-hidden ${
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center rounded-xl text-[13px] font-medium transition-all-smooth group ${
      ...
      <div className={`border-t border-figma-hairline ${isCollapsed ? "px-2 py-3" : "px-3 py-3"}`}>
        <button
          ...
          className={`w-full flex items-center rounded-xl text-[13px] text-gray-400 hover:bg-figma-surface hover:text-gray-600 transition-all-smooth ${
            isCollapsed ? "justify-center px-0 py-2.5" : "gap-3 px-3 py-2.5"
          }`}
        >
          {isCollapsed ? (
