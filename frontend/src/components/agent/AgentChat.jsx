...
import { createChatSocket } from "../../services/chatClient";
      ...
      const socket = createChatSocket({
        ...
        onStream: (chunk) => {
          setMessages((prev) => {
            if (streamMsgIdRef.current) {
              return prev.map((m) =>
                m.id === streamMsgIdRef.current ? { ...m, content: m.content + chunk } : m
              );
            }
            const id = Date.now();
            streamMsgIdRef.current = id;
            return [...prev.filter((m) => m.type !== "thinking"), { id, type: "response", content: chunk, isUser: false }];
          });
        },
        onStreamEnd: () => {
          if (streamMsgIdRef.current) {
            setMessages((prev) => {
              const msg = prev.find((m) => m.id === streamMsgIdRef.current);
        ...
        onText: (content) => {
          setMessages((prev) => {
            ...
            const responseMsg = { id: Date.now(), type: "response", content, isUser: false };
    ...
    <div className="h-full flex gap-4">
      <div className="flex-1 bg-white rounded-2xl border border-figma-hairline shadow-sm flex flex-col min-w-0">
        <div className="px-5 py-3 border-b border-figma-hairline flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-gray-400">{USE_WEBSOCKET ? "Connected to backend" : "REST mode"}</p>
          </div>
          <button onClick={handleClearHistory} className="text-[10px] text-gray-400 hover:text-gray-600 transition-all-smooth px-2 py-1 rounded-lg hover:bg-figma-surface">Clear</button>
        </div>

        {connectionError && (
          <div className="mx-5 mt-3 px-3 py-2 rounded-lg bg-amber-50 text-xs text-amber-700">{connectionError}</div>
        )}

        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 min-h-0">
          {messages.map((msg) => {
            if (msg.type === "thinking") return <ThinkingIndicator key={msg.id} message={msg.content} />;
            if (msg.type === "tool") return <ToolCard key={msg.id} tool={{ name: msg.tool }} status={msg.status} description={msg.description} />;
            if (msg.type === "action" && msg.actionType === "pipeline") {
              return <ActionCard key={msg.id} type="info" title="Pipeline Started" message={`Executing ${msg.steps.length} steps`} />;
            }
        ...
        <div className="px-5 py-3 flex-shrink-0 border-t border-figma-hairline">
          <div className="flex items-center gap-2 bg-figma-surface rounded-xl border border-figma-hairline px-4 py-2.5">
            <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="Ask your AI copilot..." disabled={isProcessing}
      ...
      <div className="hidden lg:flex w-72 bg-white rounded-2xl border border-figma-hairline shadow-sm flex-col flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {tasks.length === 0 ? (
            <div className="text-center py-8"><p className="text-xs text-gray-400">No tasks yet</p></div>
          ) : (
