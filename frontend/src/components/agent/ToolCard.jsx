const toolColors = {
  brand: { bg: "bg-brand-50", border: "border-brand-200", text: "text-brand-600", icon: "text-brand-500" },
...
export default function ToolCard({ tool, status = "running", description }) {
  const colors = toolColors[tool?.color] || toolColors.gray;
  const icon = toolIcons[tool?.icon] || toolIcons.search;

  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl border ${colors.border} ${colors.bg} fade-in`}>
      <div className={`w-8 h-8 rounded-lg ${colors.bg} flex items-center justify-center ${colors.icon}`}>
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className={`text-xs font-semibold ${colors.text}`}>{tool?.name?.replace(/_/g, " ")}</p>
        <p className="text-[10px] text-gray-500 truncate">{description || tool?.description}</p>
      </div>
