    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {skills.length > 0 ? skills.map((skill) => (
          <span key={skill} className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-[11px] font-medium">
            {skill}
          </span>
        )) : (
...
export function MissingSkills({ skills = [] }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {skills.length > 0 ? skills.map((skill) => (
          <span key={skill} className="px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 text-[11px] font-medium">
            {skill}
          </span>
        )) : (
